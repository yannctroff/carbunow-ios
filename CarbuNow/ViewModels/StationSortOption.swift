...
final class StationsViewModel: ObservableObject {
    ...
    func loadStations(in region: MKCoordinateRegion, force: Bool = false) async {
        print("🔄 [VM] loadStations(in:zone, force: \(force))")
        isLoading = true
        errorMessage = nil
        ...
        do {
            ...
            print("✅ [VM] Stations carte chargées:", fetched.count)
        } catch {
            errorMessage = error.localizedDescription
            print("❌ [VM] Erreur chargement carte:", error.localizedDescription)
        }
    ...
    func loadListStations(userLocation: CLLocation?, force: Bool = false) async {
        ...
        print("🔄 [VM] loadListStations(radius: \(searchRadiusKm) km, force: \(force))")
        isListLoading = true
        ...
        do {
            ...
            print("✅ [VM] Stations liste chargées:", fetched.count)
        } catch {
            listErrorMessage = error.localizedDescription
            print("❌ [VM] Erreur chargement liste:", error.localizedDescription)
        }
    ...
    func filteredAndSortedStations(userLocation: CLLocation?, radiusKm: Double? = nil) -> [FuelStation] {
        ...
        case .price:
            return filtered.sorted {
                let lhs = $0.price(for: selectedFuel) ?? Double.greatestFiniteMagnitude
                let rhs = $1.price(for: selectedFuel) ?? Double.greatestFiniteMagnitude
                if lhs == rhs {
        ...
        case .distance:
            return filtered.sorted {
                let lhs = $0.distance(from: userLocation) ?? Double.greatestFiniteMagnitude
                let rhs = $1.distance(from: userLocation) ?? Double.greatestFiniteMagnitude
                if lhs == rhs {
                    return $0.displayName.localizedStandardCompare($1.displayName) == .orderedAscending
                }
                return lhs < rhs
